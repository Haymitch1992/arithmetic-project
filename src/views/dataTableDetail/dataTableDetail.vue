                ...
                <el-tab-pane label="数据标注" name="first">
                    ...
                    <div class="progress-line">
                        ...
                        <span class="progress-item">
                            <el-progress :text-inside="true" :stroke-width="24" :percentage="parseFloat(progress_bar)"></el-progress>
                        </span>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 获取数据特征及预测分数获取
            getGarde() {
                this.$api.post(TRAIT_GRADE_DATA, {
            ...
            // 保存分类
            saveFormData() {
                this.$api.post(POST_ADD_CLASSIFY, {
            ...
            getClassify() {
                // 提交保住信息
                this.$api.get(GET_CLASSIFY + '?set_id=' + this.data_set_id).then((res) => {
            ...
            getChartData(locationID, data_id) {
                ...
                this.$api.post(SET_CHART_DATA, {
                    data_set_id: this.data_set_id,
                    data_id: data_id
                }).then((res) => {
                    this.myChart3 = echarts.init(document.getElementById(locationID));
                    ...
                    for (let k in res.data) {
                        if (k !== 'code') {
                            ...
                            maxLength = maxLength < res.data[k].length ? res.data[k].length : maxLength
            ...
            // 获取标注数据
            getSetData() {
                this.$api.post(GAIN_SET_DATA, {
                    data_set_id: this.data_set_id,
                    data_id: this.data_id
                }).then((res) => {
                    // // 隐藏loading
                    this.loading.close();
                    this.Set_Line_content = res.data.Set_Line_content || ['暂无数据']
                    this.set_header_list = res.data.set_header_list || ['暂无数据']
                    this.lable_type = res.data.lable_type
                    this.Len_set_lable = res.data.Len_set_lable
                    this.Len_set_line = res.data.Len_set_line
                    this.progress_bar = (parseFloat(res.data.Len_set_lable) / parseFloat(res.data.Len_set_line) * 100).toFixed(2)
                })
            },
            // 改变当前查询id
            changeSearch (str) {
                this.currentItemId = str
                this.$api.post(ALL_CLASSIFY, {
                    classify_id: str
                }).then((res) => {
            ...
            getHistogramData(obj, str) {
                ...
                this.$api.get(TRAIT_HISTOGRAM + '?data_set_id=' + this.data_set_id + '&data_trait_name=' + obj.label + objCn[str])
                    .then((res) => {
            ...
            getJsonData (str) {
                let _this = this
                this.$api.get(HISTORY_MEAN_DATA + '?data_set_id=' + this.data_set_id + '&data_name=' + str + '_mean')
                    .then(function (res) {
                        _this.myChart_history = echarts.init(document.getElementById('myChart_along'));
