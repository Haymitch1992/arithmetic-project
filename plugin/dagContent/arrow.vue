    ...
    methods: {
        ...
        r_click(e) {
            ...
            this.menu_style = Object.assign({}, this.menu_style, {
                left: `${(x - (sessionStorage['svg_left'] || 0)) /
                    this.svgScale}px`,
                top: `${(y - (sessionStorage['svg_top'] || 0)) /
                    this.svgScale}px`
            });
        ...
        computedLink() {
            ...
            if (!this.DataAll) {
                ...
                if (this.isVertical()) {
                    f_X =
                        f_Pos.pos_x +
                        (180 / (f_Pos.out_ports.length + 1)) *
                            (src_output_idx + 1);
                    f_Y = f_Pos.pos_y + 30;
                    t_X =
                        t_Pos.pos_x +
                        (180 / (t_Pos.in_ports.length + 1)) *
                            (dst_input_idx + 1);
                    t_Y = t_Pos.pos_y;
                    return `M ${f_X} ${f_Y}  Q ${f_X} ${f_Y + 50} ${(t_X +
                        f_X) /
                    ...
                    t_X = t_Pos.pos_x;
                    t_Y = t_Pos.pos_y + 15;
                    return `M ${f_X} ${f_Y}  Q ${f_X + 30} ${f_Y} ${(t_X +
                        f_X) /
                        2} ${(t_Y + f_Y) / 2} T ${t_X} ${t_Y}`;
                }
            }
        ...
        computedText() {
            ...
            if (!this.DataAll) {
                ...
                return {
                    transform: `translate(${(f_X + t_X) / 2}px, ${(f_Y + t_Y) /
                        2}px)`,
                    stroke: '#fff',
                    ...this.each.textStyle
                };
        ...
        computedArrow() {
            ...
            if (!this.DataAll) {
                ...
                let t_Y = t_Pos.pos_y;
                if (this.isVertical()) {
                    return `${t_X} ${t_Y + 3} ${t_X - 3} ${t_Y - 3} ${t_X +
                        3} ${t_Y - 3}`;
                } else {
                    let t_X = t_Pos.pos_x;
                    let t_Y = t_Pos.pos_y + 15;
                    return `${t_X - 2} ${t_Y + 3} ${t_X - 3} ${t_Y - 3} ${t_X +
                        3} ${t_Y - 1}`;
                }
            }
        },
        computedCx() {
            ...
            if (this.isVertical()) {
                f_X =
                    f_Pos.pos_x +
                    (179 / (f_Pos.out_ports.length + 1)) * (src_output_idx + 1);
            } else {
                f_X = f_Pos.pos_x + 179;
            }
            return `${f_X}`;
